...
async def generate_story(plot: PlotModel):
    ...
    paragraphs_text = group_paragraphs(paragraphs_text, 3)
    paragraphs = []
...
def generate_image(paragraph: ParagraphModel, plot: PlotModel):
    ...
    image_resp = openai.Image.create(prompt=f"generate a prompt for dall-e to represent this scene with a child book illustration: {paragraph.text} \n Having this context in mind:  {plot.main_character} \n {plot.supporting_characters}", n=1, size="512x512")
